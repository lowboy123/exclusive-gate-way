...
@RestController
@Slf4j
@Api(value = "了解 Flowable 排他网关 和 用户组 的接口")
...
public class FlowableController {
    ...
    //1、发布流程,部署好一个流程资源 , 设置好自定义的 key （个人感觉 Key 意义不是很大。。作为一个标识吧）
    ...
    @ApiOperation(value = "启动流程实例",notes = "输入流程定义的文件名称 来初始化一个流程实例，eg：xxx.bpmn.xml文件的名称,输入xxx")
    /** 这里的 userName 为实例参数用来初始化审批【部门经理】审核人员的候选组的输入方式 AA,BB,CC 每个候选人用英文逗号隔开即可 **/
    public String startProcess(@RequestParam("name") String name,@RequestParam("usersName") String usersName) {
        ...
        logger.error("pid:" + processInstance.getId() + "taskId:" + task.getId());
        return "pid:" + processInstance.getId() + ",task:" + task.toString();
    }
    ...
    public String findGroupUsers(@RequestParam("taskId") String taskId){
        logger.error("-------------->>>开始查询组任务成员列表");
        List<IdentityLink> links = taskService.getIdentityLinksForTask(taskId);
        for (IdentityLink identityLink :links) {
           logger.error("toString:"+identityLink.toString());
           logger.error("userId:"+identityLink.getUserId());
           logger.error("groupId:"+identityLink.getGroupId());
           logger.error("######################");
        }
        return "请查看日志";
    }
    ...
    @ApiOperation(value = "查询历史任务具体情况",notes = "输入一个历史任务id taskId")
    public String findHistoryGroupUsers(@RequestParam("taskId") String taskId){
        logger.error("-------------->>>开始查询历史任务具体情况");
        ...
        for (HistoricIdentityLink identityLink:historicIdentityLinks) {
            logger.error("userId:"+identityLink.getUserId());
            logger.error("groupId:"+identityLink.getGroupId());
            logger.error("createTime:"+identityLink.getCreateTime());
            logger.error("######################");
        }
        return "请查看日志";
    }
    ...
    public String SubmitReimbursementForm(@RequestParam("taskId") String taskId,@RequestParam("money") int money){
        //taskService.setVariableLocal(taskId,"报销金额",String.valueOf(money));  //关联流程实例的变量定义
        Map map = new HashMap();
        map.put("money",money);
